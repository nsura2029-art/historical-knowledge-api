-- ========================================
-- Migration 0088: People Knowledge Graph (PKG) foundation (additive only)
-- Spec: People Knowledge Graph Phase 1 Product & Data Specification v1.0
--       (2026-08-16)
--
-- This migration ADDS new tables and columns only. Existing tables
-- (claim, claim_source, claim_conflict_group, social_profile, source_record,
-- external_identifier) already cover most of the spec.
--
-- NEW tables added:
--   - person_family_relation         (Section 12 family edges, P22/P25/P26/P40/P3373)
--   - social_metric_observation      (Section 8 metric time-series, AC-14)
--   - entity_merge_history           (Section 18.5 merge audit, AC-24)
--   - entity_review_queue            (Section 18.5 ambiguous cases)
--   - attention_event                (Section 43 first-party events)
--   - identifier_scheme              (Section 11 identifier registry)
--   - claim_predicate                (Section 9 / 12 canonical predicate catalog)
--   - person_provenance_summary      (denormalized for fast profile-quality checks)
--
-- ALTERED tables:
--   - source_registry: adds ai_training_allowed, caching_rules,
--     max_retention_days, display_requirements, deletion_obligations,
--     policy_last_checked_at, policy_owner
--   - person: adds last_claim_verified_at, claim_count, source_count
--
-- NEW rows in source_registry (Tier 0-3 sources per Spec §15)
-- NEW rows in identifier_scheme (Section 11 schemes)
-- NEW rows in claim_predicate (canonical predicate vocabulary)
-- ========================================

-- ========================================
-- 1) Extend source_registry with PKG policy fields
-- ========================================
ALTER TABLE source_registry ADD COLUMN ai_training_allowed      INTEGER NOT NULL DEFAULT 0;
ALTER TABLE source_registry ADD COLUMN caching_rules           TEXT;
ALTER TABLE source_registry ADD COLUMN max_retention_days      INTEGER;
ALTER TABLE source_registry ADD COLUMN display_requirements    TEXT;
ALTER TABLE source_registry ADD COLUMN deletion_obligations    TEXT;
ALTER TABLE source_registry ADD COLUMN policy_last_checked_at  INTEGER;
ALTER TABLE source_registry ADD COLUMN policy_owner            TEXT;

CREATE INDEX IF NOT EXISTS idx_source_registry_policy_checked ON source_registry(policy_last_checked_at);

-- ========================================
-- 2) PERSON_FAMILY_RELATION — temporal family edges
-- Spec §12 family edges, §13 temporal model
-- Wikidata properties mapped:
--   P22 father → CHILD_OF (subject is child of related)
--   P25 mother → CHILD_OF
--   P26 spouse → SPOUSE_OF
--   P40 child  → PARENT_OF
--   P3373 sibling → SIBLING_OF
-- ========================================
...
-- ========================================
-- 3) SOCIAL_METRIC_OBSERVATION — time-series for follower counts etc.
-- Spec §8, §27: "Never overwrite prior observations"
-- AC-14: support latest, as-of, 7d change, 30d change
-- ========================================
...
-- ========================================
-- 4) ENTITY_MERGE_HISTORY — audit trail for entity resolution
-- Spec §18.5: "preserve merge audit trail, support split/unmerge"
-- AC-07: high-confidence merge, AC-24: auditability
-- ========================================
...
-- ========================================
-- 5) ENTITY_REVIEW_QUEUE — for ambiguous entity-resolution cases
-- Spec §18.5: "manual review in ambiguity band"
-- ========================================
...
-- ========================================
-- 6) ATTENTION_EVENT — first-party events for Phase 2 Attention Intelligence
-- Spec §43: "even though Attention is Phase 2+, begin collecting first-party events now"
-- ========================================
...
-- ========================================
-- 7) IDENTIFIER_SCHEME — registry for external IDs (Spec §11)
-- ========================================
...
-- ========================================
-- 8) CLAIM_PREDICATE — canonical predicate catalog (Spec §9, §12, §20)
-- ========================================
...
-- ========================================
-- 9) PERSON_PROVENANCE_SUMMARY — denormalized for fast queries
-- ========================================
...
-- ========================================
-- 10) Extend person table with PKG-spec fields
-- ========================================
...
-- ========================================
-- 11) Register PKG-relevant sources in source_registry (idempotent)
-- Spec §15: Tier 0 (primary) / Tier 1 (structured authority) / Tier 2 (licensed)
-- ========================================
...
-- ========================================
-- 12) Seed identifier_scheme with PKG-spec schemes
-- ========================================
...
-- ========================================
-- 13) Seed claim_predicate with canonical vocabulary
-- ========================================
...
-- ========================================
-- 14) Backfill provenance summary for existing people (initial 0/empty state)
-- ========================================
